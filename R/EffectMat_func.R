...
EffectMatrix <- function(diffCoeff, mode = "mean") {
  ...
  if (mode == "mean") {
    ...
    for (i in 1:(n_sample))
    {
      ...
      {
        diffCoeff_result[[k]] <- get_diffCoeff(scm.inter = diffCoeff$perturb[[k]], scm.ref = diffCoeff$ref[[i]], mode = mode)
        k <- k + 1
      }
    }
...
PerturbResult <- function(net_struc, data,
  ...
  if (mode == "single_cell") {
    ...
    if (deletion) {
      ...
      diffCoeff_final_result <- list(perturb = raws, ref = refs, n_sample = n_sample, n_permutation = 1)
    } else {
    ...
    diffCoeff_final_result <- list(perturb = list(scm.inter), ref = scm.ref, n_sample = 1, n_permutation = 1)
  } else {
...
#'
#' @return diffCoeff results
run_diffCoeff <- function(net_struc, data, meta, index, n_sample = 20, n_permutation = 20, diffCoeff_mode = "mean", verbal = F, replace = F) {
  ...
  if (verbal) print("Begin diffCoeff reference calculation...")
  for (i in 1:n_sample) {
  ...
  diffCoeff_final_result <- foreach::foreach(k = 1:(n_sample * n_permutation), .combine = rbind) %dopar% {
    i <- paral_index[k, "i"]
    h <- paral_index[k, "h"]
    ds_per <- ds[[i]]
  ...
  if (n_sample * n_permutation == 1) {
    ...
    diffCoeff_final_result <- list(perturb = list(diffCoeff_final_result$perturb), ref = diffCoeff_ref, n_permutation = 1, n_sample = 1)
    names(diffCoeff_final_result$perturb) <- l_name
    return(diffCoeff_final_result)
  }
  row_index <- as.vector(unlist(diffCoeff_final_result[, "k"]))
  rownames(diffCoeff_final_result) <- paste0("sample_", paral_index[row_index, "i"], "/", n_sample, "_permute_", paral_index[row_index, "h"], "/", n_permutation)
  diffCoeff_final_result <- list(perturb = diffCoeff_final_result[, "perturb"], ref = diffCoeff_ref, n_permutation = n_permutation, n_sample = n_sample)
  return(diffCoeff_final_result)
}
